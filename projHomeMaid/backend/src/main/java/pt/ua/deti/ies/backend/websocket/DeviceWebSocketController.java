...
public class DeviceWebSocketController {
    ...
    @MessageMapping("/update") // Endpoint para receber atualizações do cliente
    @SendTo("/topic/device-updates") // Tópico para broadcast
    public DeviceState handleUpdate(DeviceUpdateMessage message) throws Exception {
        // Simular um pequeno atraso no processamento
        Thread.sleep(500);

        // Aqui você faria atualizações no banco de dados ou no dispositivo real
        ...
        // Retornar o estado atualizado do dispositivo (apenas um exemplo)
